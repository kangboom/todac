...
4. **Tool을 호출하지 않는 경우 (직접 답변)**
   - "아기 돌봄, 육아, 의학적 정보와 전혀 관련이 없는 질문" (의도 분류에서 걸러지지 않은 경우)
   - 단순한 인사나 챗봇 정체성 질문
...
# =============================================================================
# 4. AnalyzeMissingInfoNode (부족한 정보 분석 및 요청)
# =============================================================================
...
[지침]
1. `response`: 사용자에게 "현재 정보만으로는 정확한 답변이 어렵다"는 점을 정중하게 알리고, 필요한 정보를 따뜻하게 물어보는 메시지를 작성하세요.
2. `missing_info`: 시스템이 추적할 수 있도록, 필요한 정보 항목들을 리스트로 추출하세요.
...
[입력 정보]
1. 원본 질문 (사용자의 최초 의도): {previous_question}
2. 시스템이 요청했던 부족한 정보 목록: {missing_info}
3. 사용자의 현재 응답 (제공된 정보): {user_response}

[지침]
1. **원본 질문의 의도를 유지**하면서, 사용자가 제공한 구체적인 정보를 질문에 자연스럽게 통합하세요.
2. 예시:
   - 원본: "아기 해열제 용량 알려줘"
   - 정보: "체중 7kg, 12개월"
